...
const headerNames = ref(["사번", "상벌명", "상벌내용", "등록일"]);
...
const validators = {
  ...
  상벌명: (value) => /^(포상|징계)$/.test(value),
  등록일: (value) => /^\d{4}-\d{2}-\d{2}$/.test(value),
};
...
const mapping = async () => {
  ...
  rowsData.value.map((row) => {
    result.value.push({
      discipline_reward_name: row["상벌명"],
      content: row["상벌내용"],
      created_at: row["등록일"],
...
const postData = async () => {
  ...
  await saveData(data, 'descipline-rewards');
  window.alert("사원 포상 및 징계 정보 등록 완료");
...
.colname {
  display: grid;
  grid-template-columns: 50px 150px 1fr 3fr 1fr;
  height: 4.5rem;
