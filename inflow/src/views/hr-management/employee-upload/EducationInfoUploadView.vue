...
const validators = {
  ...
  입학일: (value) => /^\d{4}-\d{2}-\d{2}$/.test(value),
  졸업일: (value) => /^\d{4}-\d{2}-\d{2}$/.test(value),
};
...
const mapping = async () => {
  ...
  rowsData.value.map((row) => {
    result.value.push({
      school_name: row["학교명"],
      admission_date: row["입학일"],
      graduation_date: row["졸업일"],
      degree: row["학위"],
      major: row["전공"],
...
const postData = async () => {
  ...
  await saveData(data, 'educations');
  window.alert("사원 학력 정보 등록 완료");
...
.colname {
  display: grid;
  grid-template-columns: 50px 150px 3fr 2fr 2fr 1fr 2fr;
  height: 4.5rem;
